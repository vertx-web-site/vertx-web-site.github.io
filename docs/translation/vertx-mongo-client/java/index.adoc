...
您的 Vert.x 应用可以使用 Vert.x MongoDB Client（以下简称客户端）来与 MongoDB 进行交互，
...
您还可以像下面这样，在创建一个客户端的时候指定 连接池方式数据源的名称：
...
如果您使用相同 Vert.x 实例创建了不同客户端，同时指定了相同的连接池名称，
...
但是，在有些情况下，您可能想要使用一个不给其他客户端共享连接的连接池，来创建一个客户端实例

这时，您可以使用 `link:../../apidocs/io/vertx/ext/mongo/MongoClient.html#create-io.vertx.core.Vertx-io.vertx.core.json.JsonObject-[MongoClient.create]`。
...
每次调用此方法，就相当于在调用 `link:../../apidocs/io/vertx/ext/mongo/MongoClient.html#createShared-io.vertx.core.Vertx-io.vertx.core.json.JsonObject-java.lang.String-[MongoClient.createShared]` 方法时
加上了具有唯一名称的数据源参数。
...
您可以使用 `link:../../apidocs/io/vertx/ext/mongo/MongoClient.html#save-java.lang.String-io.vertx.core.json.JsonObject-io.vertx.core.Handler-[save]` 方法来保存文档

如果文档中没有 `\_id` 字段，文档会被保存。若有，将执行 upserted。
Upserted 意思是，如果此文档不存在，就保存此文档，此文档存在就更新。

如果被保存的文档没有 `\_id` 字段，回调方法中可以获得保存后生成的 `\_id` 。
...
如果被插入的文档没有包含 `\_id`，回调方法中可以获得保存后生成的 id 。
...
必须包含 http://docs.mongodb.org/manual/reference/operator/update-field/[Update Operators] （更新运算符），
因为这些更新运算符决定了文档怎样更新。
...
您可以使用 `link:../../apidocs/io/vertx/ext/mongo/MongoClient.html#bulkWrite-java.lang.String-java.util.List-io.vertx.core.Handler-[bulkWrite]` 来一次执行多个 新增，更新，替换，或者删除操作。

在 `bulkWrite` 方法中，您可以传递 一系列 `link:../../apidocs/io/vertx/ext/mongo/BulkOperation.html[BulkOperations]`，而且每个 `BulkOperations` 运作方式 和单个操作类似。
...
这有点类似 `link:../../apidocs/io/vertx/ext/mongo/MongoClient.html#find-java.lang.String-io.vertx.core.json.JsonObject-io.vertx.core.Handler-[find]` 方法，但是仅仅返回 find 方法查询到的第一条数据。
...
例如计算 作者为Tolkien 的书的数量，结果包装在回调方法中。
...
使用这种方式，可以发挥出 MongoDB 更多优点，比如使用 MapReduce。
...
客户端可以使用MongoDB的GridFS模块来存储或检索文件和二进制数据。（译者注：GridFS是MongoDB的一个子模块，主要用于在MongoDB中存储文件，相当于MongoDB内置的一个分布式文件系统。）
...
=== 获取一个可与GridFS交互的`MongoGridFsClient`。
...
并且给方法提供一个bucket名称，可创建一个`link:../../apidocs/io/vertx/ext/mongo/MongoGridFsClient.html[MongoGridFsClient]`客户端。
在GridFS中，bucket名称最终是一个集合，该集合包含对所有存储对象的引用。
您可以通过提供一个唯一的名称将对象隔离到不同的bucket中。

它具有以下字段：

`bucketName` : 要创建的bucket的名称
...
GridFS使用 "fs" 作为默认的bucket名称。如果您想要获取默认的bucket，而不是自定义一个，
...
例如：使用默认bucket名称，获取link:../../apidocs/io/vertx/ext/mongo/MongoGridFsClient.html[MongoGridFsClient]`。
...
使用`link:../../apidocs/io/vertx/ext/mongo/MongoGridFsClient.html#uploadFileWithOptions-java.lang.String-io.vertx.ext.mongo.GridFsUploadOptions-io.vertx.core.Handler-[uploadFileWithOptions]`passing in an instance of `link:../../apidocs/io/vertx/ext/mongo/GridFsUploadOptions.html[GridFsUploadOptions]`方法，
并传入一个`link:../../apidocs/io/vertx/ext/mongo/GridFsUploadOptions.html[GridFsUploadOptions]`的实例，文件可以在存储时，携带额外到的参数。
...
通过调用`link:../../apidocs/io/vertx/ext/mongo/MongoGridFsClient.html#downloadFile-java.lang.String-io.vertx.core.Handler-[downloadFile]`方法，可以凭借文件原始名称来下载文件。
...
解析文件时可以使用文件的原始名称，下载文件时，
通过调用 `link:../../apidocs/io/vertx/ext/mongo/MongoGridFsClient.html#downloadFileAs-java.lang.String-java.lang.String-io.vertx.core.Handler-[downloadFileAs]`方法，可以重命名文件。
下载完成后，将返回下载的文件长度，该数值是`Long`类型。
...
并传入`link:../../apidocs/io/vertx/ext/mongo/GridFsUploadOptions.html[GridFsUploadOptions]`实例，可以将数据流上传到GridFS。
当数据流上传成功后，将返回GridFS生成的文件ID。
...
并传入`link:../../apidocs/io/vertx/ext/mongo/GridFsDownloadOptions.html[GridFsDownloadOptions]`实例，您可以使用文件名称并携带参数，从GridFS中下载数据流。
下载完成后，将返回下载的数据流长度，该数值是`Long`类型。
