...
然而这一切对您而言是透明的，您只需要简单地使用类似 WebSocket 的接口_即可_。
...
或者您想获得 `CDN` 版本以便在您的 `HTML` 文档中直接使用，那么首先您需要引入 sockjs 的依赖：
...
完整的使用细节可参阅https://github.com/sockjs/sockjs-client[ SockJS 网站]，
但简而言之可像这样使用：
...
router.mountSubRouter("/myapp", sockJSHandler.socketHandler(sockJSSocket -> {
  ...
  // 获取 writeHandlerID 并将其存放 (例如放在 local map 里)
