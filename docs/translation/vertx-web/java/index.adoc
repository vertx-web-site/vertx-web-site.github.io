...
它给予了您一个可以储存seession作用域信息的地方，比如购物车

Vert.x-Web 使用session cookie来识别session
这个session cookie是临时的，而且当其关闭的时候您的浏览器会将其删除

我们并不会将您session中的真实数据放到session cookie中——这个cookie只是简单的使用标记符在服务器上寻找真实的session
...
在您的处理器中您可以通过 `link:../../apidocs/io/vertx/ext/web/RoutingContext.html#session--[session]` 获取到session实例

您可以通过 `link:../../apidocs/io/vertx/ext/web/Session.html#put-java.lang.String-java.lang.Object-[put]`将数据放到session中
您可以通过 `link:../../apidocs/io/vertx/ext/web/Session.html#get-java.lang.String-[get]` 从session中获取数据
同时您也可以通过 `link:../../apidocs/io/vertx/ext/web/Session.html#remove-java.lang.String-[remove]` 从session移除数据。
...
If there is no `link:../../apidocs/io/vertx/ext/auth/User.html[User]` in the current request, then it is assumed that no previous authentication has been
