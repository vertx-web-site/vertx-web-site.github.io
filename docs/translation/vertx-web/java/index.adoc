...
我们创建了一个 HTTP 服务实例，并设置了一个请求处理器。所有的请求都会调用这个处理器处理。
...
[[_basicVertxWebconcepts]]
== Vert.x Web 的基本概念

高屋建瓴 (Here's the 10000 foot view)：

`link:../../apidocs/io/vertx/ext/web/Router.html[Router]` 是 Vert.x Web 的核心概念之一。 它是一个维护了零或多个
`link:../../apidocs/io/vertx/ext/web/Route.html[Routes]` 的对象。
...
[[_Handlingrequestsandcallingthenexthandler]]
== 处理请求并调用下一个处理器
...
[[_Routingbypathsthatbeginwithsomething]]
== 基于路径前缀的路由
