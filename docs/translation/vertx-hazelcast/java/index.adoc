...
If you are using Vert.x from the command line, the jar corresponding to this cluster manager (it will be named `vertx-hazelcast-4.3.0.jar`
...
the artifact: `io.vertx:vertx-hazelcast:4.3.0` in your project.
...
You may want to use a different version of Hazelcast. The default version is `4.2.2`. To do so, you
need to:
...
----
dependencies {
compile ("io.vertx:vertx-hazelcast:4.3.0"){
