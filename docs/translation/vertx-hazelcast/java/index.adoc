...
If you are using Vert.x from the command line, the jar corresponding to this cluster manager (it will be named `vertx-hazelcast-4.1.3.jar`
...
the artifact: `io.vertx:vertx-hazelcast:4.1.3` in your project.
...
You may want to use a different version of Hazelcast. The default version is `4.0.2`. To do so, you
need to:
...
----
dependencies {
compile ("io.vertx:vertx-hazelcast:4.1.3"){
