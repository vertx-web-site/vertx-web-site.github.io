...
Vert.x 集群管理器是可插拔的，您可以选择您想用的、或最适用于您的环境的集群管理器。
...
还可以通过配置以下系统属性，来指定 Vert.x 集群管理器 为 Ignite：
...
Ignite 默认配置使用 `TcpDiscoveryMulticastIpFinder`，因此您的网络必须保证可用组播。
