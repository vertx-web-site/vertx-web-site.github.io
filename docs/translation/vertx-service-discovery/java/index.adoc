...
这个组件提供了一套用于对各种资源服务的发布、发现的基础组件，例如 服务代理、HTTP服务节点（HTTP endpoint）、数据源（data source）... 这些资源称作 `服务` 。一个 `服务` 即是一个可被发现的功能性模块。它可以用类型、元数据、位置来区分，所以一个 `服务` 可以是一个数据库、服务代理、HTTP节点或者其他任何您能够描述、发现、交互的服务资源。它不必是一个vert.x对象，它可以是任何组件。每个 `服务` 都被描述成一个 `link:../../apidocs/io/vertx/servicediscovery/Record.html[Record]` （即：下述 `服务记录` ）。
