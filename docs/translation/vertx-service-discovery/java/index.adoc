...
----
compile 'io.vertx:vertx-service-discovery:4.0.0'
----
...
`link:../../apidocs/io/vertx/redis/client/Redis.html[Redis]` (the configuration of the client is computed from the location, metadata and
consumer configuration).
...
The client of a Redis data source service is a `link:../../apidocs/io/vertx/redis/client/Redis.html[Redis]`.
...
As stated in the previous section, how to access a data source depends on the data source itself. To build the
`link:../../apidocs/io/vertx/redis/client/Redis.html[Redis]`, you can merge configuration: the record location, the metadata and a json object provided by
the consumer:
...
When the bridge is registered the `link:../../apidocs/io/vertx/servicediscovery/spi/ServiceImporter.html#start-io.vertx.core.Vertx-io.vertx.servicediscovery.spi.ServicePublisher-io.vertx.core.json.JsonObject-io.vertx.core.Promise-[start]` method is called.
It lets you configure the bridge. When the bridge is configured, ready and has imported / exported the initial services, it must complete the given `link:../../apidocs/io/vertx/core/Future.html[Future]`.
If the bridge starts method is blocking, it must use an `link:../../apidocs/io/vertx/core/Vertx.html#executeBlocking-io.vertx.core.Handler-boolean-io.vertx.core.Handler-[executeBlocking]` construct, and complete the given future object.

When the service discovery is stopped, the bridge is stopped.
The `link:../../apidocs/io/vertx/servicediscovery/spi/ServiceImporter.html#close-io.vertx.core.Handler-[close]` method is called that provides the opportunity to cleanup resources, removed imported / exported services...
This method must complete the given `link:../../apidocs/io/vertx/core/Future.html[Future]` to notify the caller of the completion.
