...
请记住：可共用的 producer 通过 `createShared` 方法的第一次调用创建，并且它的配置在此时被设置，
可共用的 producer 使用时必须确保配置相同。

== 公用 Producer

有时你需要在多个 verticle 或上下文（context）中共享同一个 producer。
...
通过该方法返回的 produer 会共享相同的资源（线程，连接） 。

当你使用完毕该 producer 后，简单地关闭它。 当所有共用的 producer 被关闭后，所有的资源
也会被释放。
