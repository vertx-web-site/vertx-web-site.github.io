...
为了使用该组件， 需要在你的构建描述文件中的依赖配置中添加如下内容：
...
为了方便配置， 你可以将参数放置在一个 Map 容器中，并在调用
...
你也可以使用 `link:../../apidocs/java/util/Properties.html[Properties]` 来替代 Map 容器传递参数。
...
还有别的创建方法可以让你指定发送消息或接收消息的键和
数据的类型； 这样你可以直接设置键和数据的序列化/反序列化器
而不是使用属性来设置。
...
你也可以使用 `link:../../apidocs/io/vertx/kafka/client/consumer/KafkaConsumer.html#subscribe-java.util.regex.Pattern-[subscribe]` 方法去
指定一个正则表达式，并订阅所有匹配该正则表达式的主题。

为了注册一个处理器去处理接收到的消息，你需要使用
...
你可以在调用 `subscribe()` 方法的前后注册消息处理器； 直到你调用了该方法并注册了消息处理器后，消息才会
开始被消费。 举个例子，你可以先调用 `subscribe()` 方法，再调用 `seek()` 方法，最后调用 `handler()` 方法
，这样你可以在一个特定的偏移处开始消费消息。

消息处理器也可以在订阅时注册，这样你就可以获取订阅的结果并当操作完成时
收到通知。
...
你可以给 `link:../../apidocs/io/vertx/kafka/client/consumer/KafkaConsumer.html[KafkaConsumer]` 注册一个处理器，这样
...
topic, avoiding to register the handler for getting the messages and then using the `link:../../apidocs/io/vertx/kafka/client/consumer/KafkaConsumer.html#poll-java.time.Duration-io.vertx.core.Handler-[poll]` method.
