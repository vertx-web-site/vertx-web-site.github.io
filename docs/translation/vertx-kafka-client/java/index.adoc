...
你可以使用 `link:../../apidocs/io/vertx/core/streams/WriteStream.html#write-java.lang.Object-[write]` 方法去发送消息 (记录) 给主题。
...
在发送消息成功时，你可以接收到该消息在 kafka 中的元数据，例如它的主题，目标分区和它在存储中的偏移。
...
由于消息的生产者使用键的哈希计算对应的主题分区，你可以利用这一点保证拥有相同键的所有
消息都按照顺序被发送给一个相同的分区。
...
有时你需要在多个 verticle 或上下文（context）中共享同一个生产者。
...
当你使用完毕该生产者后，可以简单地关闭它。 当所有共用的生产者被关闭后，所有的资源
也会被释放。
...
关闭是异步进行的，因此在调用返回时生产者可能还没有完全关闭。 如果你想在
关闭完成时收到通知，那么你可以传入一个回调。
...
你可以调用 `link:../../apidocs/io/vertx/kafka/client/producer/KafkaProducer.html#partitionsFor-java.lang.String-io.vertx.core.Handler-[partitionsFor]` 方法来获取
指定主题的分片信息：
...
如果你是在 verticle 中创建 kafka 的消费者和生产者的，那么这些消费者和生产者会在
