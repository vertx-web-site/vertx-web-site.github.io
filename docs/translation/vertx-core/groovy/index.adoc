...
如果你正在使用Maven或者Gradle，可添加以下的依赖项到你的项目中，
...
在部署时，默认的，Vert.x将会执行该脚本。此外，你的脚本中也可以提供 `startVertx` 和 `stopVertx` 方法。相应地，这些方法将会在verticle启动或者关闭时被调用：
...
或者，你也可以继承 `link:../../apidocs/io/vertx/core/AbstractVerticle.html[AbstractVerticle]`
...
此外，你也可以重写verticle的 `stop` 方法，在verticle被反部署时该方法将会被Vert.x所调用，
...
不管你选择哪种方法去实现verticle，你都可以通过 `vertx` 变量 / 字段来访问
vert.x的实例。
...
有时你可能想在verticle启动时执行某些耗时操作，但又不想等这些操作完成后才完成verticle的部署。
例如，你可能想在start方法里部署其他的verticle。

根据黄金定律，在start方法中，你不应该阻塞以等待其他verticle部署完成。
...
经过一段时间，当你完成所有需要做的事（如：启动其他verticle），你可以在调用Promise的complete（或者fail）来表示你的事已经完成。
类似地，也有一个异步版本的stop方法。

你可以使用异步的stop去执行一些如清理verticle等耗时的操作。

当你用脚本实现verticle，异步start和stop需要这样实现:
...
当你使用类去继承 `link:../../apidocs/io/vertx/core/AbstractVerticle.html[AbstractVerticle]` ， 你需要重写
...
注意: 在verticle的stop方法中，你并不需要手动反部署子verticle。Vert.x将会在父verticle被反部署时自动反部署它的所有子verticle。
...
当使用Groovy开发vert.x应用时，你可以使用上述两个类，
...
你可以从字符串或者JSON格式的g-string创建JSON对象：
...
你可以使用 `link:../../apidocs/io/vertx/core/json/JsonObject.html#encode--[encode]`
...
你可以通过 `link:../../apidocs/io/vertx/core/json/JsonArray.html#add-java.lang.Object-[add]` 方法添加新的条目到JSON数组中。
...
你可以通过 `getXXX` 方法来读取JSON数组中的值，例如：
...
你可以使用 `link:../../apidocs/io/vertx/core/json/JsonArray.html#encode--[encode]` 方法把数组转换为字符串的形式。
