...
如果你使用 Maven 或者 Gradle， 可以通过将以下依赖添加到项目描述中的 _dependencies_ 节来使用 
...
使用 Kotlin 编写一个 Verticle 的方式与 Java 相同，继承 `link:../../apidocs/io/vertx/core/AbstractVerticle.html[AbstractVerticle]`
类即可。
...
`link:../../apidocs/io/vertx/core/json/JsonObject.html[JsonObject]` 类用来表达 JSON 对象，
`link:../../apidocs/io/vertx/core/json/JsonArray.html[JsonArray]`
类用来表达数组，可以参考 Java 文档获得更多信息。
