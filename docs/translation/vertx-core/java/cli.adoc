...
正如您所见到的一样，您可以通过 `link:../../apidocs/io/vertx/core/cli/CLI.html#create-java.lang.String-[CLI.create]` 方法来创建一个新的
 `link:../../apidocs/io/vertx/core/cli/CLI.html[CLI]` 。此处传入的字符串参数就是这个CLI的名称。创建之后，
 可以给它设置摘要和描述。一般来说，摘要是指一行简短的文字说明，
...
`link:../../apidocs/io/vertx/core/cli/Option.html#setFlag-boolean-[setFlag]` 来声明。缺省情况下，选项接收单个选项值，但是您也可以
使用 `link:../../apidocs/io/vertx/core/cli/Option.html#setMultiValued-boolean-[setMultiValued]` 将其设置成接收多个选项值：
...
非必填选项可以拥有一个默认选项值，在用户没有输入对应的选项值时，
则会启用这个默认选项值：
...
选项也可以通过 `link:../../apidocs/io/vertx/core/cli/Option.html#setHidden-boolean-[setHidden]` 方法设置成 _隐藏_ 的。隐藏的选项
不会在使用说明中显示出来，但是仍然可以起作用（提供给高级用户使用）。
...
和选项不一样，参数不以 _键_ 进行标识而是以其 _索引_ 作为标识。例如，在 
`java com.acme.Foo` 里， `com.acme.Foo` 就是一个参数。

参数没有名称，它们是以从 `0` 开始计数的索引为标识。第一个参数的
索引为 `0`：
...
`link:../../apidocs/io/vertx/core/cli/CLI.html[CLI]` 配置完成以后，您可以解析用户输入的命令行，并以此处理
每个参数和选项：
...
默认情况下，它会对用户输入的命令行进行检查校验，并确认哪些必填选项
和必填参数有无缺失，以及每个选项值的数量是否符合要求。您可以将 
...
命令行解析完成之后，您可以从 
`link:../../apidocs/io/vertx/core/cli/CLI.html#parse-java.util.List-[parse]` 方法返回的 `link:../../apidocs/io/vertx/core/cli/CommandLine.html[CommandLine]` 对象中获取到
选项值和参数值：
...
其中一个选项可以标记为“帮助”。如果命令行启用了“帮助”选项，命令行的校验
不会失败，而是有机会检查用户是否在寻求帮助：
