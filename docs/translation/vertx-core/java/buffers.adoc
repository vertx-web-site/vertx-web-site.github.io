...
任何一种情况下 Buffer都会自动进行扩容，所以你不会
在使用Buffer时遇到 `IndexOutOfBoundsException`。
...
可使用 `getUnsignedXXX`、
`appendUnsignedXXX` 和 `setUnsignedXXX` 方法将无符号数从Buffer中读取或追加/设置到Buffer里。这对于实现一个
致力于优化带宽占用的网络协议的编解码器是非常有用的。
