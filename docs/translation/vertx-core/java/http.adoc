[[_writing_http_servers_and_clients]]
== 编写 HTTP 服务端和客户端
...
Vert.x 可以处理在客户端通过 _deflate_ 或 _gzip_
算法压缩过的请求体信息。
...
NOTE: https://vertx.io/docs/vertx-web-client/java/[Vert.x Web Client] 是比HttpClient更高级别的替代品
...
HttpClient可以嵌套在非 Vert.x 线程中，如单元测试或纯Java的 `main` 线程中：
...
到 `h2c`（未加密HTTP/2服务器）的连接可能不受 HTTP 代理支持，
因为代理仅支持 HTTP/1.1。
