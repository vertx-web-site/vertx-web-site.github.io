...
一旦 `连接` 建立成功或失败，则处理器会被调用。注意：`连接` 用于建立receiver和sender。
...
消息的接收。背压协议（back-pressure protocol）是
用 http://docs.oasis-open.org/amqp/core/v1.0/csprd02/amqp-core-transport-v1.0-csprd02.html#doc-flow-control[AMQP credits] 实现的。
