...
----
compile 'io.vertx:vertx-mail-client:4.1.1'
----
...
* `poolCleanerPeriod` 整形，以毫秒为单位的连接池清理周期。默认值为 `1000 ms`。
* `poolCleanerPeriodUnit` 清理池的时间单位，默认为 `TimeUnit.MILLISECONDS`
* `keepAliveTimeout` 整形，SMTP 连接处于活跃状态（keep alive）的超时时间（以秒为单位）。默认值为`300 s`。
