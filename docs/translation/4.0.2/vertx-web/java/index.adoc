...
提供一个"附件"，附件是一个响应，它将触发浏览器打开配置为处理特定mime类型的操作系统应用程序
假设你正在生成一个PDF文件:
...
应当清楚的是，重新路由可以在 `路径` 上使用，因此，如果您需要在重新路由之间保留和/或添加状态，则应使用 `RoutingContext` 对象。
...
如果要在以不同路径根为根的不同应用程序中重用一组处handler，这也很有用。
...
你应该确保对于任何需要此功能的请求，请求体处理器都在匹配的路由上
