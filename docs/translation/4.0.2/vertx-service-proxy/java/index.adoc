...
----
compile 'io.vertx:vertx-service-proxy:4.0.2'
----
...
[source]
----
compile "io.vertx:vertx-codegen:4.0.2:processor"
compile "io.vertx:vertx-service-proxy:4.0.2"
----
...
服务调用端（客户端）可以检查它接收到的失败状态的 AsyncResult 包含的 Throwable 对象是否为 ServiceException 实例。
