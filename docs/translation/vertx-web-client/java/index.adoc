...
如果你想上传文件的同时发送属性，您可以创建一个 `link:../../apidocs/io/vertx/ext/web/multipart/MultipartForm.html[MultipartForm]` ，然后使用
...
[CAUTION]
====
默认状况下，仅当在网络级别发生错误时，Vert.x Web Client请求才以错误结尾。
换言之，`404 Not Found` 响应或错误content type的响应，*不* 被视为失败。
Use <<response-predicates, 响应谓词>> if you want the Web Client to perform sanity checks automatically.
====
...
`link:../../apidocs/io/vertx/ext/web/client/WebClient.html[WebClient]` 去做HTTP(s) 请求并且自动管理你正在调用的，
从服务器收到的所有cookie。
...
然后headers将被添加到每个请求中； 注意 这些headers将发送给所有主机；
如果你需要发送不同的headers到不同的主机， 您必须将它们手动添加到每个单个请求中，并且不添加到
