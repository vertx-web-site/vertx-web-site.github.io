...
你可以使用`UriTemplate`URI替代java字符串URI创建`HttpRequest`,
...
根据 RFC 的要求，模板扩展将用空字符串替换缺少的模板参数. 你可以修改插入失败这个情况,缺少参数后将抛出异常:
...
目标参数接受 `String`, `List<String>` 和 `Map<String, String>` .
