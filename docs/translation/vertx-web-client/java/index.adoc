...
Vert.x web 提供了一个可以缓存 HTTP 响应的实现， 您可以创建一个 `link:../../apidocs/io/vertx/ext/web/client/CachingWebClient.html[CachingWebClient]`。
...
默认情况下，带缓存的 web client 指挥缓存通过 `GET` 方法返回的状态值为 `200`, `301`, 或 `404` 的响应内容。
并且默认情况下，带有 `Vary` 头部的响应不会被缓存。
...
该 web client 是一个 `link:../../apidocs/io/vertx/ext/web/client/WebClientSession.html[WebClientSession]`。 请参阅 <<_handling_private_responses>> 。

[[_providing_an_external_store]]
=== 使用别的储存实现

默认情况下， 带缓存的客户端使用一个本地的 `Map` 来保存响应。 您也可以使用您自己的储存实现类来保存响应内容。
您需要实现 `link:../../apidocs/io/vertx/ext/web/client/spi/CacheStore.html[CacheStore]` 接口，并且在创建客户端时指定使用您自己的实现类。
...
`link:../../apidocs/io/vertx/ext/web/client/WebClient.html[WebClient]` 去做HTTP(s) 请求并且自动管理您正在调用的，
从服务器收到的所有cookie。
...
如果您需要发送不同的 headers 到不同的主机， 您必须将它们手动添加到每个单个请求中，并且不添加到
