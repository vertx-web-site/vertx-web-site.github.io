...
一旦您创建了这个对象，您就可以注册或者注销例程。在后面的章节中会给出更多详情。
...
您可以直接在该对象中注册例程。
在任何时刻都可以进行例程的注册和注销，即使在路由注册完成之后也可以：
...
此处的例程是指一个检查系统某个切面的函数，用于推断当前的健康状况。
...
当您注册了一个例程，您需要对其命名，并且要指定一个函数（handler）来执行该项检查。
