...
健康检查组件可以单独使用，也可以和 Vert.x Web 或者 event bus 联合使用。

该组件提供一个 Vert.x Web handler 让您可以注册一些规程
用于测试应用程序的健康状况。
该 handler 计算出最终状态并以 JSON 方式返回结果。
...
一旦您创建了这个对象，您就可以注册或者注销规程。在后面的章节中会给出更多详情。
...
可以直接在 `link:../../apidocs/io/vertx/ext/healthchecks/HealthCheckHandler.html[HealthCheckHandler]` 对象实例中注册规程。
此外，如果您已经预先创建了 `link:../../apidocs/io/vertx/ext/healthchecks/HealthChecks.html[HealthChecks]` 对象的实例，
您可以直接在该对象中注册规程。
在任何时刻都可以进行规程的注册和注销，即使在路由注册完成之后也可以：
...
此处的规程是指一个检查系统某个切面的函数，用于推断当前的健康状况。
它报告一个 `link:../../apidocs/io/vertx/ext/healthchecks/Status.html[Status]` 对象用以指示该测试通过还是发生故障。
该函数将检查结果报告给它所对应的 `link:../../apidocs/io/vertx/core/Promise.html[Promise]` ，并且请注意该函数不可以阻塞这个 `link:../../apidocs/io/vertx/core/Promise.html[Promise]` 。

当您注册了一个规程，您需要对其命名，并且要指定一个函数（handler）来执行该项检查。

推断健康状况的规则如下：

* 如果对应的 promise 被标记为故障，则检查结果认定为 _KO_
...
规程可以进行分组管理。 规程的名称里可以指定分组信息。 分组的规程按照树形结构进行组织，
...
当使用 Vert.x web handler 时，总体健康检查信息通过对外公开的 
`link:../../apidocs/io/vertx/ext/healthchecks/HealthCheckHandler.html[HealthCheckHandler]` 的路由路径，
以 HTTP 的 GET 或者 POST 请求（取决于您注册该路由的方式）来获取

如果没有注册任何规程， 则响应信息为 `204 - NO CONTENT` ， 表明系统状态为 _UP_ 但是没有执行任何规程。
此时响应信息不包含任何有效数据。

如果注册了至少一个规程，该规程将被执行并计算出结果。
响应码包括下列几种：

* `200` : 一切正常
* `503` : 至少有一个规程报告了不健康状态
* `500` : 某个规程抛出了错误，或者未能及时报告状态

响应的内容是一个 JSON 文档，体现的是总体结果（`outcome`）。总体结果要么是 `UP` 要么是 `DOWN` 。
此外还给出了一个 `checks` 数组用以显示每个执行过的规程的结果。
如果某个规程报告了额外的数据，这些数据也会一并给出：
...
如果采用了分组/层级结构，则 `checks` 数组通过以下结构进行描述：
...
如果一个规程抛出了错误，或者报告了故障（异常），该 JSON 文档会在 `data` 字段下给出 `cause` 字段。
如果某个规程未能及时上报结果，则结果将显示为 `Timeout` （超时）。
