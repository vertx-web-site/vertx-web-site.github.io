...
----
dependencies {
 compile 'io.vertx:vertx-web:4.1.7'
}
----
...
您需要添加 `io.vertx：vertx-auth-jwt：4.1.7` 依赖项才能使用 `JWTAuthProvider`。
...
`io.vertx:vertx-web-templ-mvel:4.1.7`。 并通过此方法以创建 MVEL 模板引擎实例：
...
`io.vertx:vertx-web-templ-jade:4.1.7`。 并通过此方法以创建 Jade 模板引擎实例：
...
`io.vertx:vertx-web-templ-handlebars:4.1.7`。 并通过此方法以创建 Handlebars 模板引擎实例：
...
`io.vertx:vertx-web-templ-thymeleaf:4.1.7`。 并通过此方法以创建 Thymeleaf 模板引擎实例：
...
`io.vertx:vertx-web-templ-freemarker:4.1.7`。 并通过此方法以创建 Apache FreeMarker 模板引擎实例：
...
`io.vertx:vertx-web-templ-pebble:4.1.7`。 并通过此方法以创建 Pebble 模板引擎实例：
...
您需要在项目中添加以下 _依赖_ 以使用 Rocker：`io.vertx:vertx-web-templ-rocker:4.1.7`。
...
`io.vertx:vertx-web-templ-httl:4.1.7`。 并通过此方法以创建 HTTL 模板引擎实例：
...
`io.vertx:vertx-web-templ-rythm:4.1.7`。 并通过此方法以创建 Rythm 模板引擎实例：
...
注意：`SOCKET_CLOSED` 事件可能会包含消息。在这种情况下检查type属性，可能会引入一种新的
