...
JSON Web 令牌是一种简单的发送明文信息（通常是URL）的方法，
其内容可被验证为是可信的。 
像下面的这些场景 JWT 是非常适用的：
...
这便是最简单的加载秘钥方式。你只需要将 secret 与第三方共享，举个例子
...
在这种情况下secret将被设置为公钥，因为这是双方都知道的令牌，你可以配置 PubSec 密钥为对称的。
...
如果你更倾向于使用 Java Keystores 格式的秘钥文件，那么您也可如此做。
