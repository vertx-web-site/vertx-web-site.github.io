...
schema是一个由 `link:../../apidocs/io/vertx/json/schema/common/Validator.html[Validator]` 对象组成的树，树中的每一个元素都包含了必要的校验逻辑。
这种校验具有 快速失败 的特性：一旦遇到验证错误，验证就会失败而不再继续

每一个已解析的schema叫做 `link:../../apidocs/io/vertx/json/schema/Schema.html[Schema]`，
