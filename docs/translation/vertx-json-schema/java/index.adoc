...
如果你想要使用代码来构建schemas，你可以使用附带的DSL。只有Draft-7支持这个特性。
If you want to build schemas from code, you can use the included DSL. Only Draft-7 is supported for this feature.

首先，静态导入 `link:../../apidocs/io/vertx/json/schema/draft7/dsl/Schemas.html[Schemas]` 和 `link:../../apidocs/io/vertx/json/schema/draft7/dsl/Keywords.html[Keywords]` 。
...
使用属性schemas和其他属性schema来创建一个对象类型的schemas：
...
你也可以使用别名来引用通过dsl定义的schemas。你可以使用 `link:../../apidocs/io/vertx/json/schema/common/dsl/SchemaBuilder.html#alias-java.lang.String-[alias]` 来为一个schema设置别名。
这样你就可以使用 `link:../../apidocs/io/vertx/json/schema/common/dsl/Schemas.html#refToAlias-java.lang.String-[Schemas.refToAlias]` 根据别名来引用schema。
