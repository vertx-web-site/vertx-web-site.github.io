...
这种校验具有快速失败的特性：一旦遇到校验错误，校验就会直接失败而不再继续
...
对于不同的 `SchemaParser` ， 你可以复用同一个 `SchemaRouter` 实例，并且你还可以使用同一个的 `SchemaParser` 来解析不同的 `Schema` 。
...
如果你想要使用代码来构建schemas，你可以使用附带的DSL。目前只有Draft-7支持这个特性。
