...
如果您使用了 `send` （而不是 `publish` ），并且 Camel exchange 期望一个回复 （ _In Out_ exchange），
Vert.x 代码应作为对已发送消息的回复。当收到回复消息时，它钭传播到 exchange：
...
如果您在事件总线上发送消息时注册了回复的处理器，则它将 Camel exchange 配置为期望一个响应
