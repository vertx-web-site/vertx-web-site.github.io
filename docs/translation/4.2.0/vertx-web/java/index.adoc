...
----
dependencies {
 compile 'io.vertx:vertx-web:4.2.0'
}
----
...
您也可以使用 `link:../../apidocs/io/vertx/ext/web/RoutingContext.html#data--[data]` 获取全部的上下文数据map
...
您需要添加 `io.vertx：vertx-auth-jwt：4.2.0` 依赖项才能使用 `JWTAuthProvider`。
...
`io.vertx:vertx-web-templ-mvel:4.2.0`。 并通过此方法以创建 MVEL 模板引擎实例：
...
`io.vertx:vertx-web-templ-jade:4.2.0`。 并通过此方法以创建 Jade 模板引擎实例：
...
`io.vertx:vertx-web-templ-handlebars:4.2.0`。 并通过此方法以创建 Handlebars 模板引擎实例：
...
`io.vertx:vertx-web-templ-thymeleaf:4.2.0`。 并通过此方法以创建 Thymeleaf 模板引擎实例：
...
`io.vertx:vertx-web-templ-freemarker:4.2.0`。 并通过此方法以创建 Apache FreeMarker 模板引擎实例：
...
`io.vertx:vertx-web-templ-pebble:4.2.0`。 并通过此方法以创建 Pebble 模板引擎实例：
...
您需要在项目中添加以下 _依赖_ 以使用 Rocker：`io.vertx:vertx-web-templ-rocker:4.2.0`。
...
`io.vertx:vertx-web-templ-httl:4.2.0`。 并通过此方法以创建 HTTL 模板引擎实例：
...
`io.vertx:vertx-web-templ-rythm:4.2.0`。 并通过此方法以创建 Rythm 模板引擎实例：
