...
如果你想要通过 WebSockets 来进行连接，可以通过 `link:../../apidocs/io/vertx/mqtt/MqttServerOptions.html[MqttServerOptions]` 来启用它，
...
与其他连接的配置方式一样，由此产生的 endpoint 的连接以及断开与常规的连接管理方式保持一致。
