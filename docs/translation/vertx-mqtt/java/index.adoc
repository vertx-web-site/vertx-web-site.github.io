...
实际上，尽管仅有一个MQTT服务器被部署，但是当传入的连接到达时，会被Vert.x使用轮转算法分发到不同的核上运行的handlers上。
...
这里我们有一个使用 `link:../../apidocs/io/vertx/mqtt/MqttClient.html#subscribe-java.lang.String-int-[subscribe]` 方法的例子，
为了接收到主题为 rpi2/temp 的消息，我们调用了 `link:../../apidocs/io/vertx/mqtt/MqttClient.html#subscribe-java.lang.String-int-[subscribe]` 方法，
因此，为了能接收到的服务端的消息，你需要提供一个handler，每当你订阅的主题有新的消息传来，这个handler就会被调用。
