...
更多详情可以在这里找到 https://vertx.io/docs/vertx-core/java/#_native_transports[Vert.x documentation].
...
比如, 您可以通过添加 `search_path` 属性来配置一个默认的schema.
...
关于连接uri字符串格式的更多信息可以在这里找到 https://www.postgresql.org/docs/9.6/static/libpq-connect.html#LIBPQ-CONNSTRING[PostgreSQL Manuals]
...
如果您没有在连接时指定连接对象或者URI字符串, 此时将会使用到环境变量.
...
注意 SCRAM-SHA-256-PLUS(在Postgresql 11加入)当前版本客户端暂不支持.
...
注意: PostreSQL会在事务结束后销毁游标, 因而游标API应该在事务内使用,
...
channel()方法的参数即通道名称(接收端)需要和PostgreSQL发送通知时的通道名称保持一致.
