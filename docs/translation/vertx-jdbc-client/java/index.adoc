...
Verx.x JDBC client 允许您使用Vert.x应用程序中的异步API
与任何符合JDBC协议的数据库进行交互。
...
因此JDBC客户端实现了相同的API，直接使用JDBC驱动,实现数据库的异步支持。
...
创建客户端后,你可以通过 `link:../../apidocs/io/vertx/ext/sql/SQLClient.html#getConnection-io.vertx.core.Handler-[getConnection]` 
来获取一个连接
...
长时间保持客户端(例如，在你的verticle的生命周期内)是可以的，
但是一旦完成，就应该关闭它。

与其他客户端实例共享数据源的客户端被计数。
...
当执行查询时，
返回结果通过 `link:../../apidocs/io/vertx/ext/sql/ResultSet.html[ResultSet]` 回调处理
...
而实际结果通过 `link:../../apidocs/io/vertx/ext/sql/ResultSet.html#getResults--[getResults]`
...
- 作为Json对象实例的列表来检索行 
- 这可以使您使用的API稍微简单一些，但是请注意，SQL结果可以包含重复的列名-如果是这种情况，则应
...
这将进行查询，其中包含参数占位符和一个 `link:../../apidocs/io/vertx/core/json/JsonArray.html[JsonArray]` 
或参数值
