...
----
compile 'io.vertx:vertx-jdbc-client:4.2.6'
----
...
* The `encoder` `link:../../apidocs/io/vertx/ext/jdbc/spi/JDBCEncoder.html[JDBCEncoder]` to convert `Java input type` to `SQL type`. By default implementation, it is `io.vertx.ext.jdbc.spi.impl.JDBCEncoderImpl`
* The `decoder` `link:../../apidocs/io/vertx/ext/jdbc/spi/JDBCDecoder.html[JDBCDecoder]` to convert `SQL result type` to `Java type`. By default implementation, it is `io.vertx.ext.jdbc.spi.impl.JDBCDecoderImpl`
