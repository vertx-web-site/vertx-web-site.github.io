...
使用 `CallableStatement` 接口。 对于大多数存储因此能够来说，这种抽象是合二为一的，因为像
`PostgreSQL` or `MySQL` 这种流行的数据库并没有任何特殊的命令以区分普通的 SQL 语句和
可调用的函数与存储过程。
...
在这种情况下，JDBC 客户都安可以使用一个叫做 `link:../../apidocs/io/vertx/jdbcclient/SqlOutParam.html[SqlOutParam]` 的帮助类，它可以帮您显式地
