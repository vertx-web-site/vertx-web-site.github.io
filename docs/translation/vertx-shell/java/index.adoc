...
----
vertx run -conf '{"telnetOptions":{"port":5000}}' maven:io.vertx:vertx-shell:4.0.3
----
...
# 启动 shell
vertx run -conf '{"sshOptions":{"port":4000,"keyPairOptions":{"path":"ssh.jks","password":"secret"},"authOptions":{"provider":"shiro","config":{"properties_path":"file:auth.properties"}}}}' maven:io.vertx:vertx-shell:4.0.3
----
...
你可以使用 `service:io.vertx.ext.shell` 来代替 `maven:io.vertx:vertx-shell:4.0.2`
...
当 shell 运行时，
`link:../../apidocs/io/vertx/ext/shell/term/Pty.html[Pty]` 伪终端是与命令进行交互的主要手段：
...
当一个远程终端进行连接时，
`link:../../apidocs/io/vertx/ext/shell/term/Term.html[Term]` 可以用来与当前连接的终端进行交互。
