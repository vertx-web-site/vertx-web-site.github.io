...
NOTE: 在默认情况下，当您使用 `DataObjects` 时， `base64` 字符串会使用 `base64url` 字母表处理，而 OpenAPI 并不强制使用这个字母表，因此它默认使用 `basic` 字母表进行处理。为了强制 `DataObject` 使用一个指定的字母表，您需要在 `@DataObject` 注解中配置对应的参数。
