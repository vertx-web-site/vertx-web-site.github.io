...
Vert.x Web API 服务通过事件总线帮助你处理HTTP请求

事件总线提供了一些重要特性，例如：负载均衡、通过不同Vert.x实例分发请求
我们建议您查看 https://vertx.io/docs/vertx-core/java/#event_bus[Event Bus documentation] 来了解更多信息

这个模块提供给你创建Web API 服务的能力，即一个事件总线消息消费者(基于与 https://vertx.io/docs/vertx-service-proxy/java/[Vert.x service proxy] 同样的概念 )
...
你需要引入 `vertx-codegen` 来从被注解修饰的接口中生成代码。
如果你只需要 `link:../../apidocs/io/vertx/ext/web/api/service/RouteToEBServiceHandler.html[RouteToEBServiceHandler]` ，那么你不需要 `vertx-codegen`。

如果你希望用不同语言来调用接口，那么你需要添加 _语言_ 依赖，譬如：
为groovy语言添加 `vertx-lang-groovy` 依赖.

[[_proxy_an_http_request_to_a_web_api_service]]
== 代理一个HTTP请求 到一个Web API服务

你可以用 `link:../../apidocs/io/vertx/ext/web/api/service/RouteToEBServiceHandler.html[RouteToEBServiceHandler]` 来代理发送到事件总线的请求。
...
您也可以定义 `link:../../apidocs/io/vertx/core/eventbus/DeliveryOptions.html[DeliveryOptions]` ，这个option会被用在每次通过事件总线发送消息的时候：
...
IMPORTANT: 在挂载 `link:../../apidocs/io/vertx/ext/web/api/service/RouteToEBServiceHandler.html[RouteToEBServiceHandler]` 的时候，您 *必须* 同时挂载一个 `link:../../apidocs/io/vertx/ext/web/validation/ValidationHandler.html[ValidationHandler]` ，这个ValidationHandler提取了请求当中的参数。否则，在发送请求的时候不回存在任何请求参数。
...
现在我们可以构建用于处理那些终端（endpoint）的 *TransactionService* 接口。对于各个终端 您在构建 `link:../../apidocs/io/vertx/ext/web/api/service/RouteToEBServiceHandler.html[RouteToEBServiceHandler]` 的时候 需要写一个方法，这个方法名称与指定的 `action` 相关联。
对于方法参数，有如下几个规则：
...
当你从 `TransactionService#getTransactionsList` 方法中接收到一个请求，自动生成的服务处理器会从 `link:../../apidocs/io/vertx/ext/web/api/service/ServiceRequest.html[ServiceRequest]` 自动解析出 `from` 和 `to` 参数（如果存在）

服务处理器也有自动转换 `JsonObject` 为Vert.x 数据对象的能力，例如，如果你有一个满足上述json schema的 `Transaction` 数据对象，你可以像如下重写 `putTransaction` 方法签名：
...
你也可以用 `link:../../apidocs/io/vertx/ext/web/validation/RequestParameter.html[RequestParameter]` 来提取参数，如下：
...
要写一个请求 你必须调用包含了 `link:../../apidocs/io/vertx/ext/web/api/service/ServiceResponse.html[ServiceResponse]` 的 `resultHandler`
...
`ServiceRequest` 是一个 _可序列化_ 的 `RoutingContext` ，但是 它并不包含 `RoutingContext` 的所有数据。它将如下数据转送到你的服务：
...
你可以用 `link:../../apidocs/io/vertx/ext/web/api/service/RouteToEBServiceHandler.html#extraPayloadMapper-java.util.function.Function-[extraPayloadMapper]` 配置一个lambda表达式来构建额外的 payload
...
* 响应头
* 状态码/状态信息
* 作为payload的请求体。如果你不设置payload或者设置为null，则不会发送响应体。
