...
----
compile 'io.vertx:vertx-circuit-breaker:4.2.3'
----
...
当然这个也是可以配置的，您可以调用方法
`link:../../apidocs/io/vertx/circuitbreaker/CircuitBreakerOptions.html#setNotificationAddress-java.lang.String-[setNotificationAddress]` 
来设置总线通知地址。如果您设置为 `null` ，那么总线通知将被禁用。
...
如果您使用了Hystrix异步方法, 对应回调函数是不会在 Vert.x 的线程中执行的,因此我们必须在执行前保持上下的引用,
