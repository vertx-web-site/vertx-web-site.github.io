...
* 你可以用 `link:../../apidocs/io/vertx/ext/web/api/validation/ParameterTypeValidator.html[ParameterTypeValidator]` 的静态方法来实例化您的自定义预置类型的实例，然后用后缀是 `WithCustomTypeValidator` 的函数将它加载到 `link:../../apidocs/io/vertx/ext/web/api/validation/HTTPRequestValidationHandler.html[HTTPRequestValidationHandler]`
