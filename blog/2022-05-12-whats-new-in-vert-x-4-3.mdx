...
In vert.x web, one of the pain points by users is to figure out the right order for handlers. In 4.3 when adding
multiple handlers under the same route, the order will be verified and a report will be signaled if the setup is
incorrect.
...
All builtin handlers are now classified as:

* PLATFORM (enhancement of the platform, such as logging)
* SECURITY_POLICY (HTTP security policies such as CSP, HSTS, etc...)
* BODY (Body parsing related handlers)
* AUTHENTICATION (should be self explanatory)
* INPUT_TRUST (input validation)
* AUTHORIZATION (should be self explanatory)
* USER (any other handler)

If you have a handler that performs any of these tasks and want to make use of the runtime validation all it is required
is to mark your implementation with one of the marker interfaces. If no marker interface is used, then it's always
assumed to be a user handler.

#### Safe security callback setup

Also, all security handlers that require callbacks, will now ensure that the callbacks are added in the right order, so
no need to create temporary objects anymore:
...
Before you would need to create a reference to the callback route before the oauth2 handler was added otherwise the
callback handler would be shaded.

This feature applies to all security handler that use callbacks, for example: `OAuth2`, `OTP` and `Webauthn`.

#### Other notorious improvements
