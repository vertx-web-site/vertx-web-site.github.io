...
HCL Domino is the latest incarnation of the venerable [groupware](https://en.wikipedia.org/wiki/Collaborative_software) originally known as [Lotus Notes](https://en.wikipedia.org/wiki/HCL_Notes). It is mainly written in C/C++ for its NoSQL database and core functionality, with an Java layer for some of the server functionality. The Domino REST API (DRAPI) is written in Java using Vert.x and sits on top of the core functionality.
...
DRAPI not only provides the REST API into Domino, but also can host single page applictions (SPA). To ensure their protection, we use the [CSP Handler](https://vertx.io/docs/vertx-web/java/#_csp_handler) to enforce a [Content Security Policy](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP) on the SPAs. Not all developers follow a clear separation of HTML, CSS and JavaSript, so we allow different levels of CPS depending on each SPA. The Vert.x [Router](https://vertx.io/docs/apidocs/io/vertx/ext/web/Router.html) allows to mount individual CSP policies on each route.

Most of our routes are protected using [JWT](https://jwt.io/). To accomodate our customers' deployment situations, we build our own IdP implementation on top of [Vert.x Auth](https://vertx.io/docs/vertx-auth-common/java/), as well as added support for [OIDC](https://openid.net/developers/how-connect-works/) using Vert.x' [OAuth2 auth provider](https://vertx.io/docs/vertx-auth-oauth2/java/). It has been successsfully tested with both [Keycloak](https://www.keycloak.org) and [Microsoft Entra ID](https://www.microsoft.com/en-us/security/business/identity-access/microsoft-entra-id) (formerly known as MS Azure Active Directory).
...
Last not least Vert.x' [Scheduler](https://vertx.io/docs/vertx-core/java/#_scheduled_tasks) keeps our IdP certificates updated.
...
- flexible loading of configuation
...
- friendly and reposive community and maintainers

You can checkout the [entire documentation](https://opensource.hcltechsw.com/Domino-rest-api/).