...
Let's refresh our mind about what we developed so far in the _introduction to vert.x_ series. In [the first post](/blog/my-first-vert-x-3-application/), we developed a very simple Vert.x 3 application, and saw how this application can be tested, packaged and executed. In [the second post](/blog/vert-x-application-configuration/), we saw how this application became configurable and how we can use a random port in test, and use another configurable port in production. Finally, the [previous post](/blog/some-rest-with-vert-x/) has shown how to use vertx-web and how to implement a small REST API. However, we forgot an important task. We didn't test the API. In this post we will increase the confidence we have on this application by implementing unit and integration tests.
...
Let's start slowly. Remember in the first post we have implemented a unit test with [vertx-unit](/docs/vertx-unit/java/). The test we did is dead simple:
...
In the [next post](/blog/using-the-asynchronous-sql-client/), we replace the _in memory_ backend with a database, and use asynchronous integration with this database.
