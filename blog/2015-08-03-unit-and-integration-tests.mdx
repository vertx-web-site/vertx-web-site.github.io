...
export const meta = {
  ...
  summary: "Let’s refresh our mind about what we developed so far in the introduction to vert.x series. We forgot an important task. We didn’t test the API."
}

Let's refresh our mind about what we developed so far in the _introduction to vert.x_ series. In <Link href="/blog/[[...slug]]" as="/blog/my-first-vert-x-3-application/" passHref={true}><a>the first post</a></Link>, we developed a very simple Vert.x 3 application, and saw how this application can be tested, packaged and executed. In <Link href="/blog/[[...slug]]" as="/blog/vert-x-application-configuration/" passHref={true}><a>the second post</a></Link>, we saw how this application became configurable and how we can use a random port in test, and use another configurable port in production. Finally, the <Link href="/blog/[[...slug]]" as="/blog/some-rest-with-vert-x/" passHref={true}><a>previous post</a></Link> has shown how to use vertx-web and how to implement a small REST API. However, we forgot an important task. We didn't test the API. In this post we will increase the confidence we have on this application by implementing unit and integration tests.
...
Let's start slowly. Remember in the first post we have implemented a unit test with <Link href="/docs/[[...slug]]" as="/docs/vertx-unit/java/" passHref={true}><a>vertx-unit</a></Link>. The test we did is dead simple:
...
<Alert info title="Is the fatjar created?">
...
<Alert info title="Am I ready to serve?">

You may need to wait in the `configureRestAssured` method that the HTTP server has been started. We recommend the [awaitility](https://github.com/jayway/awaitility) test framework to check that the request can be served. It would fail the test if the server does not start.
...
In the <Link href="/blog/[[...slug]]" as="/blog/using-the-asynchronous-sql-client/" passHref={true}><a>next post</a></Link>, we replace the _in memory_ backend with a database, and use asynchronous integration with this database.
